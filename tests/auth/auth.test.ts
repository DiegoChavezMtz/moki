...
function setup(overrides: Partial<Gateway> = {}) {
  const calls: unknown[] = [];
  const gateway = {
    signUp: async (input: unknown) => { calls.push(input); return { data: { user: { id: "user-1", email: "uno@example.test" }, session: { access_token: "test" } }, error: null }; },
...
test("registro directo guarda el nombre visible sin solicitar un correo", async () => {
  const { service, calls } = setup();
  assert.deepEqual(await service.register(" Ana ", " uno@example.test ", "password"), { id: "user-1", email: "uno@example.test" });
  assert.deepEqual(calls, [{ email: "uno@example.test", password: "password", options: { data: { name: "Ana" } } }]);
});
...
test("registro informa cuando Supabase aún exige confirmación por correo", async () => {
  const { service } = setup({ signUp: async () => ({ data: { user: { id: "user-1", email: "uno@example.test" }, session: null }, error: null }) } as unknown as Partial<Gateway>);
  await assert.rejects(service.register("Ana", "uno@example.test", "password"), /registro directo todavía no está configurado/);
});
...
test("un error desconocido de registro no se expone", async () => {
  const { service } = setup({ signUp: async () => ({ data: {}, error: { code: "unexpected", message: "secret" } }) } as unknown as Partial<Gateway>);
  await assert.rejects(service.register("Ana", "uno@example.test", "password"), { message: "No pudimos completar la solicitud. Inténtalo de nuevo." });
});
