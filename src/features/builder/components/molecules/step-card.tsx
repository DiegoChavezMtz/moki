...
export function StepCard({ step, index, count, busy, active, error, onInstruction, onMove, onRemove, onDrag, onDragEnd, file, onFile }: Props & { file?: File; onFile?: (file: File | null) => void }) {
  ...
  return <article className={`step-card ${active ? "active-pulse" : ""} ${error ? "invalid" : ""}`} data-step-id={step.id} style={{ "--step-color": block.color } as CSSProperties} aria-label={`Bloque ${index + 1}: ${block.label}`}>
    ...
    <div className="step-body"><div className="step-head"><span className="step-type-label">{index + 1}. {block.label}</span><button className="icon-btn" disabled={busy} onClick={onRemove} aria-label={`Eliminar bloque ${index + 1}`}>×</button></div>
