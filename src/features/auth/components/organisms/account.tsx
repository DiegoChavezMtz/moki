...
export function Account() {
  ...
  return <><h1>Mi cuenta</h1>{loading ? <p role="status">Comprobando sesión…</p> : user ? <><p className="auth-subtitle">{user.email}</p><label className="auth-field">Nombre visible<input value={name} maxLength={120} onChange={(event) => setName(event.target.value)} disabled={busy} /></label><button className="auth-submit" disabled={busy} onClick={() => void saveProfile()}>{busy ? "Un momento…" : "Guardar nombre"}</button><Link href="/cambiar-contrasena">Cambiar contraseña</Link><button className="auth-submit" disabled={busy} onClick={() => void signOut()}>{busy ? "Un momento…" : "Cerrar sesión"}</button><hr /><p className="auth-subtitle">Eliminar tu cuenta borrará tus agentes. Los forks de otras personas se conservarán.</p>{confirmDeletion ? <><button className="auth-submit" disabled={busy} onClick={() => void removeAccount()}>{busy ? "Eliminando…" : "Sí, eliminar mi cuenta definitivamente"}</button><button className="auth-submit" disabled={busy} onClick={() => setConfirmDeletion(false)}>Cancelar</button></> : <button className="auth-submit auth-danger" disabled={busy} onClick={() => setConfirmDeletion(true)}>Eliminar cuenta</button>}</> : <p className="auth-subtitle">Inicia sesión para acceder a tu cuenta.</p>}
